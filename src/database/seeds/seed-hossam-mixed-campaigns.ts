...
const publicCampaignSeeds: PublicCampaignSeed[] = [
  {
    baseName: 'حملة عامة - أحذية رياضية',
    description: 'إطلاق مجموعة أحذية رياضية بتصميم عصري وجودة عالية',
    ...
    contentDescription: 'محتوى عملي يبرز الراحة والتصميم خلال تجربة يومية',
  ...
  {
    baseName: 'حملة عامة - مشروب طاقة',
    description: 'الترويج لمشروب طاقة جديد بنكهات مبتكرة',
    implementationType: ImplementationType.REMOTE_PHOTOGRAPHY,
    includedPlatforms: [
    ...
    contentDescription: 'تجربة المنتج خلال أنشطة رياضية وعمل وحياة يومية',
...
const privateCampaignSeeds: PrivateCampaignSeed[] = [
  {
    baseName: 'حملة خاصة - تطبيق تنظيم وقت',
    description: 'تعاون حصري للترويج لتطبيق إنتاجية لتنظيم اليوم',
    implementationType: ImplementationType.REMOTE_PHOTOGRAPHY,
    includedPlatforms: [TargetPlatform.YOUTUBE, TargetPlatform.INSTAGRAM],
    contentTypes: [ContentTypeOffer.YOUTUBE_VIDEO, ContentTypeOffer.REEL],
    contentDescription: 'شرح ميزات التطبيق وكيف يساعد على تنظيم اليوم',
    influencerType: InfluencerType.MICRO,
  ...
  {
    baseName: 'حملة خاصة - حقيبة سفر ذكية',
    description: 'تعاون حصري لإبراز مميزات حقيبة سفر بمواصفات احترافية',
    implementationType: ImplementationType.REMOTE_PHOTOGRAPHY,
    includedPlatforms: [TargetPlatform.INSTAGRAM, TargetPlatform.TIKTOK],
    contentTypes: [ContentTypeOffer.REEL, ContentTypeOffer.STORY],
    contentDescription: 'استعراض تصميم الحقيبة وميزاتها العملية أثناء السفر',
    influencerType: InfluencerType.MICRO,
...
async function createPrivateCampaign(
  ...
  const campaign = campaignRepo.create({
    ...
    submittedAt: new Date(),
    approvedAt: new Date(),
    implementationStartDate: formatDateOnly(implementationStartDate) as unknown as Date,
    implementationEndDate: formatDateOnly(implementationEndDate) as unknown as Date,
  });
...
async function run(): Promise<void> {
  ...
  console.log(`\nCreating ${publicCampaignSeeds.length} public campaigns...`);
  for (const seed of publicCampaignSeeds) {
    await createPublicCampaign(seed, advertiser.id, categoryId, uniqueSuffix);
  }

  console.log(
    `\nCreating ${privateCampaignSeeds.length} private campaigns + invitations for ${INFLUENCER_EMAIL}...`,
  );
